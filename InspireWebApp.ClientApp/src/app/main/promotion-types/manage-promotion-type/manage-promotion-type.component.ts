...
import {PromotionTypesClient, PromotionTypeCreateModel, PromotionTypeUpdateModel} from "../../../@core/app-api";
import {PromotionTypeRepresentingService} from "../../../@core/promotion-types/promotion-type-representing.utils";
import {InputTextModule} from "primeng/inputtext";

interface PromotionTypeCreateForm {
  name: FormControl<string>;
}

interface PromotionTypeUpdateForm {
  name: FormControl<string>;
}
...
export class ManagePromotionTypeComponent implements OnInit {
  ...
  form!: FormGroup<PromotionTypeCreateForm> | FormGroup<PromotionTypeUpdateForm>;
  ...
  ngOnInit(): void {
    ...
    if (routeData.hasOwnProperty('item')) {
      ...
      const itemResult: ApiResult<PromotionTypeUpdateModel> = routeData['item'];
  ...
  private addItem(): void {
    const form = <FormGroup<PromotionTypeCreateForm>>this.form;

    const model = new PromotionTypeCreateModel({
      name: form.controls.name.value,
    });

    this.promotionTypesClient.create(model)
      .pipe(
  ...
  private updateItem(): void {
    const form = <FormGroup<PromotionTypeUpdateForm>>this.form;

    const model = new PromotionTypeUpdateModel({
      name: form.controls.name.value,
    });

    this.promotionTypesClient.update(this.itemId!, model)
      .pipe(
        this.formDisabler.monitor.monitor(),
        this.crudHelpers.handleManageToasts(
          this.representingService
  ...
  private buildCreateForm(): FormGroup<PromotionTypeCreateForm> {
      ...
      name: new FormControl('', {
        ...
        asyncValidators: [
          CommonValidators.commonUniqueValidator(abstractControl => {
            ...
            return this.promotionTypesClient.checkUniqueName(this.itemId, value);
          }),
        ],
      }),
    });
  ...
  private buildUpdateForm(): FormGroup<PromotionTypeUpdateForm> {
    return new FormGroup({
      name: new FormControl('', {
        nonNullable: true,
        validators: [
...
@Injectable()
export class EditPromotionTypeTitleProvider extends EditItemTitleProvider<ManagePromotionTypeComponent> {
  getItemName$(component: ManagePromotionTypeComponent): Observable<string | undefined> {
