...
import {ProductCategoriesClient, ProductCategoryDetailsModel} from "../../../@core/app-api";
import {
...
export class ViewProductCategoryComponent implements OnInit {
  item!: ProductCategoryDetailsModel;
  ...
  constructor(
    ...
    protected readonly representingService: ProductCategoryRepresentingService,
  ) {
  }
  ...
  ngOnInit(): void {
    this.item = (this.activatedRoute.snapshot.data['item'] as ApiResult<ProductCategoryDetailsModel>).value!;
  ...
  delete() {
    this.productCategoriesClient.delete(this.item.id)
      .pipe(
...
@Injectable()
export class ViewProductCategoryTitleProvider extends ResolvedApiItemTitleProvider<ProductCategoryDetailsModel> {
  private readonly representingService = inject(ProductCategoryRepresentingService);

  getItemTitle(item: ProductCategoryDetailsModel): Changeable<string> {
